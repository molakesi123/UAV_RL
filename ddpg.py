...
class DDPG:
    ...
    def update(self):
        if len(self.memory) < self.batch_size:  # 当 memory 中不满足一个总量时，不更新策略
            return
        ...
        torch.nn.utils.clip_grad_norm_(parameters=self.actor.parameters(), max_norm=5, norm_type=2)
        ...
        value_loss = nn.MSELoss()(value, expected_value.detach())  # 均方差损失函数


        self.critic_optimizer.zero_grad()
        value_loss.backward()
        torch.nn.utils.clip_grad_norm_(parameters=self.critic.parameters(), max_norm=5, norm_type=2)
